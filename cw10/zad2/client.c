...
int main(int argc, char *argv[]) {
    ...
    if(strcmp(argv[2], "unix") == 0) {
        ...
        int option = 1;
        if(setsockopt(server_sock, SOL_SOCKET, SO_PASSCRED, &option, sizeof(int)) != 0){perror("Cannot set sockopt");exit(1);}
        ...
        if(bind(server_sock, (struct sockaddr*)&addr, sizeof(sa_family_t)) != 0){perror("Cannot bind");exit(1);}
    ...
    int type = MESSAGE_HELLO;

    if(write(server_sock, &type, sizeof(int)) != sizeof(int)) {
        perror("cant send message type to server\n");
        exit(1);
    }
